...
// ═══════════════════════════════════════════════════════════════════════════════
export default function Analisis8020View({
  showToast,
  userRole,
  selectedStores = [],
  manualData = [],
  ...
  const filteredProspecciones = useMemo(() => {
    let filtered = isAdmin 
      ? (selectedStores.length > 0 ? manualData.filter(r => selectedStores.includes(String(r.Tienda || '').trim().toUpperCase())) : manualData)
      : manualData.filter(r => selectedStores.includes(String(r.Tienda || '').trim().toUpperCase()));
    return filtered.map(r => {
      return { ...r, Perdidos: parseInt(r.Perdidos) || 0 };
    });
  }, [manualData, isAdmin, selectedStores]);
  ...
  const handleSyncManual = () => {
    setIsSyncing(true);
    if (showToast) showToast('Sincronizando datos...', 'info');
    // App.jsx refreshes globally on interval or manual refresh
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between',
        ...
        <div>
          <p className="section-label">Ventas — Análisis 80/20 Mensual</p>
